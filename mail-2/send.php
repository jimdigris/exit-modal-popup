...
try {
    ...
    //$mail->addAddress(RECIPIENTS_EMAIL_2);
    $mail->addAddress(RECIPIENTS_EMAIL_3);
...
// записываем данные из формы в файл
writeDataFile ($name, $phone, $email, $message);
function writeDataFile ($name, $phone, $email, $message) {
    date_default_timezone_set('Europe/Moscow');
    $path = $_SERVER['DOCUMENT_ROOT'] . '*******/base.csv';
    $dateTime = date("Y-m-d");
    $str = array('аккумуляторный', $dateTime, $name, $phone, $email, $message);
    $fp = fopen($path, 'a');
    fputcsv($fp, $str, ';');
    fclose($fp);
}
